...
export class TracksController {
  ...
  @Post()
  createTrack(@Body() body: CreateTrackDto, @Res({ passthrough: true }) res: Response) {
  ...
  @Get(':id')
  getTrackById(@Param('id') id: string, @Res({ passthrough: true }) res: Response) {
    if (!validate(id)) {
  ...
  @Put(':id')
  updateTrackInfo(@Param('id') id: string, @Body() body: CreateTrackDto, @Res({ passthrough: true }) res: Response,) {
    if (!validate(id)) {
      res.status(StatusCodes.BAD_REQUEST).send('Track id is invalid');
      return;
    }
